...
pub fn devices(_app_attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let shared_statics: Vec<_> = shared_names
        .iter()
        .map(|n| quote::format_ident!("__{}__", n.to_string().to_uppercase()))
        .collect();
    ...
    let nonshared_statics: Vec<_> = nonshared_names
        .iter()
        .map(|n| quote::format_ident!("__{}__", n.to_string().to_uppercase()))
        .collect();
    ...
    let device_statics: Vec<_> = device_names
        .iter()
        .map(|n| quote::format_ident!("__{}__", n.to_string().to_uppercase()))
        .collect();
    // Temporary idents used inside __init_devices__ to hold the static shared ref
    let device_static_refs: Vec<_> = device_names
        .iter()
    ...
    let out = quote! {
        pub mod devices {
            ...
            #(
                pub static #shared_statics: ::oreos::prelude::static_cell::StaticCell<
            ...
            #(
                pub static #nonshared_statics: ::oreos::prelude::static_cell::StaticCell<#nonshared_types>
                    = ::oreos::prelude::static_cell::StaticCell::new();
            )*

            // MaybeDevice items — stored in statics so start() gets a &'static self
            #(
                pub static #device_statics: ::oreos::prelude::static_cell::StaticCell<#device_types>
                    = ::oreos::prelude::static_cell::StaticCell::new();
            )*
